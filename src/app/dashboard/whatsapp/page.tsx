    ...
    return (
        <div className="max-w-4xl mx-auto pb-20">
            <header className="mb-10">
                ...
                <p className="text-sm text-slate-600 font-medium">Gerencie a conexão do seu número com o agente de IA.</p>
            </header>
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                        <div className="flex items-center gap-5">
                            ...
                            <div>
                                ...
                                <div className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-bold border ${state === 'open' ? 'bg-emerald-50 text-emerald-700 border-emerald-100' : state === 'connecting' ? 'bg-amber-50 text-amber-700 border-amber-100' : 'bg-slate-50 text-slate-500 border-slate-100'}`}>
                                    <span className={`w-2 h-2 rounded-full ${state === 'open' ? 'bg-emerald-500' : state === 'connecting' ? 'bg-amber-400 animate-pulse' : 'bg-slate-300'}`}></span>
                            ...
                            {state === 'open' ? (
                                <>
                                    <button
                                        ...
                                        className="flex items-center gap-2 px-5 py-3 bg-slate-50 text-slate-900 font-bold text-sm rounded-xl hover:bg-slate-100 transition-all disabled:opacity-50 border border-slate-200"
                                    >
            ...
            {qrCode && (
                <div className="bg-slate-50 rounded-3xl border border-slate-200 overflow-hidden mb-6">
                    <div className="p-8 text-center">
                        ...
                        <p className="text-sm text-slate-600 font-medium mb-8">Abra o WhatsApp → Menu (⋮) → Aparelhos Conectados → Conectar.</p>
                        <div className="inline-block p-4 bg-white rounded-2xl border border-slate-200 shadow-xl">
                            <img src={qrCode} alt="QR Code WhatsApp" className="w-64 h-64" />
                        </div>
                        <p className="text-[10px] text-slate-400 font-bold mt-6 uppercase tracking-widest">O QR Code expira em 60 segundos. Gere outro se necessário.</p>
                    </div>
                </div>
            )}
        </div>
