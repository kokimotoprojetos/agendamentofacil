            ...
            <div className="container px-4 mx-auto">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-6xl mx-auto">
                    {plans.map((plan: any, index: number) => (
                        <div
                            key={index}
                            className={`relative glass p-10 rounded-[2.5rem] border-slate-200 flex flex-col transition-all hover:-translate-y-2 ${index === 1 ? 'border-orange-500/30 shadow-2xl shadow-orange-500/10 scale-105 z-10' : ''}`}
                        >
                            {index === 1 && (
                            ...
                            <a
                                href="/register"
                                className={`w-full py-4 rounded-2xl font-bold text-center transition-all ${index === 1 ? 'bg-orange-600 text-white hover:bg-orange-500 shadow-lg shadow-orange-500/20' : 'bg-slate-100 text-slate-900 hover:bg-slate-200 border border-slate-200'}`}
                            >
                                Assinar Agora
                            </a>
                        </div>
